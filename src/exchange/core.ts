...
import { updatePairDayData, updateTokenDayData, updateSpookyDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let spooky = SpookyFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  spooky.totalLiquidityFTM = spooky.totalLiquidityFTM.minus(pair.trackedReserveFTM as BigDecimal);
  ...
  spooky.totalLiquidityFTM = spooky.totalLiquidityFTM.plus(trackedLiquidityFTM);
  spooky.totalLiquidityUSD = spooky.totalLiquidityFTM.times(bundle.ftmPrice);
...
export function handleMint(event: Mint): void {
  ...
  spooky.totalTransactions = spooky.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let spooky = SpookyFactory.load(FACTORY_ADDRESS);
  spooky.totalVolumeUSD = spooky.totalVolumeUSD.plus(trackedAmountUSD);
  spooky.totalVolumeFTM = spooky.totalVolumeFTM.plus(trackedAmountFTM);
  spooky.untrackedVolumeUSD = spooky.untrackedVolumeUSD.plus(derivedAmountUSD);
  spooky.totalTransactions = spooky.totalTransactions.plus(ONE_BI);
  ...
  let spookyDayData = updateSpookyDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  spookyDayData.dailyVolumeUSD = spookyDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  spookyDayData.dailyVolumeFTM = spookyDayData.dailyVolumeFTM.plus(trackedAmountFTM);
  spookyDayData.dailyVolumeUntracked = spookyDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  spookyDayData.save();
